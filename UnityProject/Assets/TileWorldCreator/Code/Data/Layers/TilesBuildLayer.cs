...
namespace GiantGrey.TileWorldCreator
{
    ...
    public class TilesBuildLayer : BuildLayer, ISerializationCallbackReceiver
    {
        ...
        public class TilePresetOverride
        {
            ...
            public TilePreset preset;
            /// <summary>
            /// Only when using DualGrid. If true, it will only return an override if tile position has 4 cell positions 
            /// </summary>
            [Range(1, 4)]
            [HelpBox("Only when using DualGrid. If true, it will only return an override if tile position has 4 cell positions ")]
        ...
        IEnumerator GenerateTilesParallel(List<Vector2> positions, BlueprintLayer _blueprintLayer)
        {
                ...
                if (useDualGrid)
                {
                    for (int x = -1; x < 1; x++)
                    {
                        for (int y = -1; y < 1; y++)
                        {
                            ...
                            ProcessTileParallel(cellPosition, worldPosition, newTilesBag, existingCellPositions);
                        }
                    }
                }
                else
                {
                    ProcessTileParallel(worldPosition, worldPosition, newTilesBag, existingCellPositions);
                }
            });

            // Convert ConcurrentBag to List efficiently
            // List<TileData> _newTiles = new List<TileData>(newTilesBag.Count);
            ...
            // Update worldGrid using a ConcurrentDictionary
            var concurrentWorldGrid = new ConcurrentDictionary<int, Dictionary<Vector2, bool>>();

            Parallel.ForEach(positions, worldPosition =>
            {
                int worldGridHashmap = GetClusterKeyFromPosition(worldPosition);

                // Ensure dictionary entry exists
                var subGrid = concurrentWorldGrid.GetOrAdd(worldGridHashmap, _ => new Dictionary<Vector2, bool>());
            ...
            // Merge concurrent dictionary into main worldGrid (single-threaded)
            foreach (var kvp in concurrentWorldGrid)
            {
                if (!worldGrid.TryGetValue(kvp.Key, out var subGrid))
                {
                    worldGrid[kvp.Key] = kvp.Value;
                }
                else
                {
                    foreach (var entry in kvp.Value)
                    {
                        subGrid[entry.Key] = entry.Value;
                    }
                }
            }
        ...
        void ProcessTileParallel(Vector2 cellPosition, Vector2 worldPosition, ConcurrentBag<TileData> newTilesBag, HashSet<Vector2> existingCellPositions)
        {
            lock (existingCellPositions)
            {
            ...
            newTile.isAssigned = true;
            // newTile.isNew = animatedPositions.Contains(worldPosition);
            newTile.tilePosition = cellPosition;
            newTile.worldMapPosition = worldPosition;
            ...
            lock (tileGrid)
            {
                ...
                if (subGrid.TryGetValue(cellPosition, out var existingTile))
                {
                    ...
                    subGrid[cellPosition] = existingTile; // Reassign the modified struct back to the dictionary
                }
                else
                {
        ...
        // {
        //     if (animatedPositions.Contains(_position))
        //     {
        //         animatedPositions.Remove(_position);
        //     }
                    ...
                    if (_meshFilter.sharedMesh.vertexCount > 0)
                    {
                        switch (_colliderType)
                        {
                            ...
                            case Configuration.ColliderType.tileCollider:
                                ...
                                var _layer = currentBlueprintLayer;
                                ...
                                if (_collisionMesh.vertexCount > 0)
                                {
                                    var _meshCollider2 = _cluster.AddComponent<MeshCollider>();
                                    _meshCollider2.cookingOptions = MeshColliderCookingOptions.WeldColocatedVertices;
                        ...
                        //                         if (addGrassRenderer)
                        //                         {
                        //                             var _grassRenderer = _cluster.AddComponent<PointGrassRenderer>();
                        //                             _grassRenderer.bladeType = PointGrassCommon.BladeType.Mesh;
                        //                             // _grassRenderer.renderLayer.Set(1<<6);
                        //                             _grassRenderer.multipleMeshes = true;
                        //                             _grassRenderer.multipleMaterials = true;
                        //                             _grassRenderer.grassBladeMeshes = new Mesh[2];
                        //                             _grassRenderer.grassBladeMeshes[0] = grassBladeMesh_A;
                        //                             _grassRenderer.grassBladeMeshes[1] = grassBladeMesh_B;
                        //                             _grassRenderer.meshDensityValues = new float[2];
                        //                             _grassRenderer.meshDensityValues[0] = grassMeshAWeight;
                        //                             _grassRenderer.meshDensityValues[1] = grassMeshBWeight;
                        //                             _grassRenderer.materials = new Material[2];
                        //                             _grassRenderer.materials[0] = grassMaterial_A;
                        //                             _grassRenderer.materials[1] = grassMaterial_B;
                        //                             _grassRenderer.shadowMode = grassShadowCastingMode;
                        //                             _grassRenderer.pointCount = grassPointCount;
                        //                             _grassRenderer.multiplyByArea = true;
                        //                             _grassRenderer.randomiseSeed = false;
                        //                             _grassRenderer.seed = 123123;
                        //                             _grassRenderer.BuildPoints();
            ...
            for (var t = 0; t < tileLayers[_tileLayerIndex].layerOverrides.Count; t++)
            {
                ...
                if (_layer != null)
                {
                    ...
                    if (useDualGrid)
                    {
                        ...
                        // if (tilePresetOverrides[t].requireFourNeighbours && _count == tilePresetOverrides[t].requiredNeighbourCount)
                        {
